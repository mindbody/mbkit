...
exports.createPages = ({ graphql, actions }) => {
  ...
  `).then(result => {
    ...
    const components = result.data.allContentfulComponent.edges.sort(
      (a, b) => a.node.componentName[0] > b.node.componentName[0]
    )
    ...
    components.forEach(edge => {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            `⚠️ There was an error fetching the changelog for ${componentName}`
          )
        }

        // generating slug (checking for forward slash and adding one if it doesn't exist)
        const componentSlug =
          slug[0] === "/" ? `/components${slug}` : `/components/${slug}`

        // throwing component into component navigation
        componentNav.push({
          title: componentName,
          to: componentSlug,
        })
    ...
    fs.writeFile(
      "generated/componentNav.json",
      JSON.stringify(componentNav),
      err => {
        if (err) {
          throw err
        }
